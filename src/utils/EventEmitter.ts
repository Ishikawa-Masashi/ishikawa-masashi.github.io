...
export default class EventEmitter {
   ...
   */
  off(eventType: any, listenerFunc: any) {
    ...
    if (hasType) {
      if (!listenerFunc) {
        ...
        removedEvents.forEach(function (idx: number) {
   ...
   * @protected
   */
  _applyEvents(eventType: string, eventArguments: any) {
    const typeListeners = this._listeners[eventType];
    ...
    const removableListeners: any = [];
    typeListeners.forEach(function (eeListener: any, idx: number) {
    ...
    removableListeners.forEach(function (idx: number) {
      typeListeners.splice(idx, 1);
    });
  }
   ...
   * @param eventArgs
   */
  emitSync(type: string, ...eventArgs: any[]) {
    this._applyEvents(type, eventArgs);
  }
